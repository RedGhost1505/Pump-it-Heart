...
def calcular_angulo_entre_vectores(vector1, vector2):
    ...
    magnitude1 = m.sqrt(vector1[0] ** 2 + vector1[1] ** 2)
    magnitude2 = m.sqrt(vector2[0] ** 2 + vector2[1] ** 2)
    cosine_angle = dot_product / (magnitude1 * magnitude2)
    angle = m.degrees(m.acos(cosine_angle))
...
if _name_ == "_main_":
    ...
    while cap.isOpened():
        ...
        if not success:
            print("No se pudo leer el frame de la cámara. Fin del stream.")
            break
        # Get fps.
