...
# Definir el tiempo mínimo necesario dentro del botón para activar la acción (en segundos)
tiempo_minimo_dentro_botón = 1  # Por ejemplo, 3 segundos

# Inicializar el temporizador de tiempo dentro del botón
time_inside_button = 0


# Obtener la frecuencia de cuadros por segundo del video
...
# Definir el tiempo mínimo necesario dentro del botón para activar la acción (en segundos)
tiempo_minimo_dentro_botón = 3  # Por ejemplo, 3 segundos

# Bandera para rastrear si el sonido ya se ha reproducido
sound_played = False
...
while cap.isOpened():
    ...
    if success:
        ...
        canva = np.zeros(esqueleto.shape, np.uint8)
        ...
        if results.pose_landmarks:
            ...
            #-------------------------------------------------------------
            # #Pendiente del pie derecho
            # pendiente1 = (y30 - y32) / (x30 - x32) if (x30 - x32) != 0 else 0

            # # Define un umbral para considerar la línea como nivelada.
            # # Este valor es un ejemplo y puede necesitar ajustes.
            # umbral_pendiente1 = 0.1

            # # Determina si la línea está nivelada comparando la pendiente con el umbral
            # esta_nivelada1 = abs(pendiente1) < umbral_pendiente1

            # # Visualiza el resultado
            # texto_estado1 = 'Nivelada' if esta_nivelada1 else 'No Nivelada'
            # color_texto1 = (0, 255, 0) if esta_nivelada1 else (0, 0, 255)

            # cv2.putText(esqueleto, f'Linea Pie Derecho: {texto_estado1}', (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, color_texto1, 2)
            # #-------------------------------------------------------------
            # #Pendiente del pie izquierdo
            # pendiente2 = (y31 - y29) / (x31 - x29) if (x31 - x29) != 0 else 0

            # # Define un umbral para considerar la línea como nivelada.
            # # Este valor es un ejemplo y puede necesitar ajustes.
            # umbral_pendiente2 = 0.1

            # # Determina si la línea está nivelada comparando la pendiente con el umbral
            # esta_nivelada2 = abs(pendiente2) < umbral_pendiente2

            # # Visualiza el resultado
            # texto_estado2 = 'Nivelada' if esta_nivelada2 else 'No Nivelada'
            # color_texto2 = (0, 255, 0) if esta_nivelada2 else (0, 0, 255)

            # cv2.putText(esqueleto, f'Linea Pie Izquierdo: {texto_estado2}', (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, color_texto2, 2)
            #-------------------------------------------------------------
            ...
            cv2.putText(esqueleto, 'CD', (x1, y1), cv2.FONT_HERSHEY_PLAIN, 1, (0, 206, 208), 2)
           
            cv2.circle(esqueleto, (x2, y2), 6, (255,0,0),cv2.FILLED)#rightshoulder
            cv2.putText(esqueleto, 'RD', (x2, y2), cv2.FONT_HERSHEY_PLAIN, 1, (0, 206, 208), 2)
            
            cv2.circle(esqueleto, (x3, y3), 6, (255,0,0),cv2.FILLED)#rightelbow
            cv2.putText(esqueleto, 'TD', (x3, y3), cv2.FONT_HERSHEY_PLAIN, 1, (0, 206, 208), 2)

            cv2.circle(esqueleto, (x11, y11), 6, (255,0,0),cv2.FILLED)#righthip
            cv2.putText(esqueleto, 'CI', (x11, y11), cv2.FONT_HERSHEY_PLAIN, 1, (0, 206, 208), 2)

            cv2.circle(esqueleto, (x22, y22), 6, (255,0,0),cv2.FILLED)#rightshoulder
            cv2.putText(esqueleto, 'RI', (x22, y22), cv2.FONT_HERSHEY_PLAIN, 1, (0, 206, 208), 2)

            cv2.circle(esqueleto, (x33, y33), 6, (255,0,0),cv2.FILLED)#rightelbow
            cv2.putText(esqueleto, 'TI', (x33, y33), cv2.FONT_HERSHEY_PLAIN, 1, (0, 206, 208), 2)
            ...
            # Mostrar la imagen con los botones y las acciones
