...
class SecondaryWindow(QMainWindow):
    ...
    def update_frame(self):
        success,esqueleto = self.cap.read()
        if success:
            height, width, _ = esqueleto.shape 
            results = self.pose.process(esqueleto)
            if results.pose_landmarks:
                
                self.mp_drawing.draw_landmarks(esqueleto,results.pose_landmarks,self.mp_pose.POSE_CONNECTIONS, landmark_drawing_spec=self.mp_drawing.DrawingSpec(color=(255, 255, 255), thickness=2, circle_radius=2), connection_drawing_spec=self.mp_drawing.DrawingSpec(color=(245,66,230), thickness=2, circle_radius=2))
                ...
                if self.estado_anterior_flag is None:
                    self.estado_anterior_flag = FLAG
                else:
                    # Si el estado de la flag ha cambiado, incrementar el contador
                    if FLAG != self.estado_anterior_flag:
                        self.estado_anterior_flag = FLAG
                        if self.estado_anterior_flag == True:
                            self.contador_cambios += 1
                            # self.estado_anterior_flag = FLAG
                
                print(angle, angle2, Indicator, self.contador_cambios)
                ...
                cv2.putText(canva, f'Contador de repeticiones: {self.contador_cambios}', (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_texto, 1)
                cv2.putText(esqueleto, f'Contador de repeticiones: {self.contador_cambios}', (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, color_texto, 2)
                cv2.putText(esqueleto, f'Linea Manos: {texto_estado}', (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_texto, 1)


            # Convierte el frame a formato adecuado para PyQt
            esqueleto = cv2.cvtColor(esqueleto, cv2.COLOR_BGR2RGB)
            h, w, ch = esqueleto.shape
            bytesPerLine = ch * w
            convertToQtFormat = QImage(esqueleto.data, w, h, bytesPerLine, QImage.Format.Format_RGB888)  # Corrección aquí
            p = convertToQtFormat.scaled(self.ui.imageLabel.width(), self.ui.imageLabel.height(), QtCore.Qt.AspectRatioMode.KeepAspectRatio)
            self.ui.imageLabel.setPixmap(QPixmap.fromImage(p))
            self.ui.label_5.setText(f'{self.contador_cambios}')
